...
import { getPolicyNavBySlug } from '../data/policiesNav'

export function PolicyModulePage() {
  const { slug } = useParams<{ slug: string }>()

  const item = useMemo(
    () => (slug ? getPolicyNavBySlug(slug) : undefined),
    [slug],
  )
        ...
        <div className="flex flex-wrap items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-slate-600 dark:text-slate-400">
              This screen is a frontend placeholder. Connect your API and
              replace this content when the module is ready.
            </p>
          </div>
        </div>
      </div>
